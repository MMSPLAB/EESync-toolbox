# acquisition/spike_demo.py
# Randomized spike marker generator for demo sync integration.
...
# ====== RUNNER ======
class _PeriodicSpikeRunner:
    """Run a background thread that periodically triggers spikes.

    Emits SYNC.trigger_spike(label, source). Stops via stop().
    """
    ...
    def __init__(self, name: str, delay_range_sec: Tuple[float, float], labels: Iterable[str]) -> None:
        ...
        self._labels = list(labels) or ["SPIKE"]       # Available labels
        self._stop = threading.Event()                 # Cooperative stop flag
        self._thr = threading.Thread(            # Daemon thread drives emission loop
            target=self._loop, name=f"SpikeDemo:{self._name}", daemon=True
        )

    def start(self) -> "_PeriodicSpikeRunner":
        """Start the background thread."""
        logger.info("Starting spike demo emitter '%s'", self._name)
        self._thr.start()
        return self
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            pass
        logger.info("Stopped spike demo emitter '%s'", self._name)
    ...
    def _loop(self) -> None:
        ...
        while not self._stop.is_set():
            try:
                ...
                SYNC.trigger_spike(label, self._name)     # Instant trigger
            except Exception:
                pass                                      # Keep running
...
def _lookup_config_delay_range(name: str) -> Optional[Tuple[float, float]]:
    ...
    generators = CONFIG.get("marker_generators", {}).get("spike_demo", {}).get("INSTANCES", [])
...
# ====== PUBLIC API ======
def start_spike_demo(
    name: str,
    interval_s: Optional[float] = None,
    labels: Optional[Iterable[str]] = None,
    delay_range_sec: Optional[Tuple[float, float]] = None,
):
    """Start a single spike demo runner and return it.

    If labels is None, use SPIKE_KEYMAP values.
    """
    if labels is None:
        vals = (CONFIG.get("spikes", {}).get("SPIKE_KEYMAP", {}) or {}).values()  # Configured labels
        labels = [str(v) for v in vals] or ["SPIKE"]  # Use config or default name
    ...
    return _PeriodicSpikeRunner(name=name, delay_range_sec=delay_range, labels=labels).start()
